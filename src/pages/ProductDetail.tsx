      ...
      <div className="container py-8">
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 lg:gap-12">
              ...
              <div className="flex gap-2">
                {product.images.map((img, i) => (
                  <button
                    ...
                    className={`w-16 h-16 rounded-lg overflow-hidden border transition-colors ${i === selectedImage ? "border-foreground" : "border-border"}`}
                  >
          ...
          <div className="space-y-6">
            ...
            <div className="flex items-center gap-3">
              ...
              <Badge variant="outline" className="rounded-full border-foreground/20 text-sm">{product.conditionScore}/10</Badge>
              {product.isCombo && <Badge className="rounded-full bg-foreground text-background">COMBO</Badge>}
            </div>
            ...
            {inStock && (
              <div className="flex items-center gap-4">
                ...
                <Button onClick={handleAddToCart} className="h-11 px-6 rounded-button bg-foreground text-background hover:bg-foreground/90 gap-2">
                  <ShoppingCart className="h-4 w-4" /> Add to Cart
                </Button>
                <Button onClick={handleBuyNow} variant="outline" className="h-11 px-6 rounded-button border-border">
                  Buy Now
                </Button>
              </div>
            )}
                  ...
                  <div key={r.id} className="flex gap-3 p-4 rounded-card border border-border bg-card">
                    ...
                    <div>
                      <div className="flex items-center gap-2">
                        ...
                        <div className="flex gap-0.5">{Array.from({ length: r.rating }).map((_, i) => <Star key={i} className="h-3 w-3 fill-foreground text-foreground" />)}</div>
                      </div>
        ...
        {related.length > 0 && (
          <section className="mt-16">
            <h2 className="text-2xl font-semibold text-foreground mb-6">Related Products</h2>
